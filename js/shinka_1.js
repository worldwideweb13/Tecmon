...
// "アプリ制作に取りかかる"押下時、コメント表示のイベント発火、及びステータスの更新処理を行う
document.getElementById("CreateButton").addEventListener("click",function(){
    ...
    //加算されるスコア計算 
    let Fcalc = Monster["FrontScore"] + SkillCalc(Monster["FrontSkill"]);    
    Monster["FrontScore"] = Fcalc;
    let Mcalc = Monster["BackScore"] + SkillCalc(Monster["BackSkill"]);    
    Monster["BackScore"] = Mcalc;
    let Icalc = Monster["InfraScore"] + SkillCalc(Monster["InfraSkill"]);    
    Monster["InfraScore"] = Icalc;
    //計算結果の画面挿入 
    InsertText("CurrentHp", Monster["MonsterHp"]);
    InsertText("CurrentFrontScore", Monster["FrontScore"]);
    InsertText("CurrentBackScore", Monster["BackScore"]);
    ...
    document.getElementById("FrontScoreBar").style.width = fc + "%";
    document.getElementById("BackScoreBar").style.width = bc + "%";
    document.getElementById("InfraScoreBar").style.width = ic + "%";
    // FBデータ更新
    db.ref('/users/' + UserId + '/' + MonsterName).update(Monster);
});
...
// アプリ制作をする押下時のコメント欄
function CreatePrintMoji() {
    switch(mi){
        case mi=0:
            m = MonsterName + "はアプリ制作を始めた！";
            document.getElementById("Next").setAttribute('onclick','CreatePrintMoji(); TextReading()');
            document.getElementById("Next").style.display="block"            
            mi++;
            break;
        case mi=1:
            m = MonsterName + Monster["CreateComment_1"];
            ;
            mi++;
            break;
        case mi=2:
            m = MonsterName + Monster["CreateComment_2"];
            mi++;
            break;
        case mi=3:
            if(Monster["MonsterHp"] == 0){
            ...
            m = MonsterName + "に何をさせる？";
            document.getElementById("Next").removeAttribute('onclick','CreatePrintMoji()');
            mi=0;
            Evolution();
            break;           
    }
